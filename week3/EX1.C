...
INT8U rdsearch(INT8U* myRdyTbl, INT8U* myRdyGrp) {
    while (1) {
        int rand_num;
        rand_num = random(64);
        if ((myRdyTbl[rand_num >> 3] & myMapTbl[rand_num & 0x07]) != myMapTbl[rand_num & 0x07]) {
            *myRdyGrp |= myMapTbl[rand_num >> 3];
            myRdyTbl[rand_num >> 3] |= myMapTbl[rand_num & 0x07];
            return rand_num;
        }
    }
}
  ...
  INT8U column = 0;
    ...
    for (j = 0; j < 4; j++) {
      ...
      while (1)
      {
        ...
        for (i = 0; i < 4; i++) {
          rand_num[i] = rdsearch(myRdyTbl, &myRdyGrp);
          OSTimeDly(10);
          PC_DispChar(x++, y, '0' + (rand_num[i] / 10), DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
          PC_DispChar(x++, y, '0' + (rand_num[i] % 10), DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
          x++;
        }
        // find min (of 4 random numbers)
        min = (myUnMapTbl[myRdyGrp] << 3) + (myUnMapTbl[myRdyTbl[myUnMapTbl[myRdyGrp]]]);
